...
///////////////// FINE CONFIG 
 ...
 TString name = "~/Desktop/MmumuJ_2016"; //nome del file salvato

/////input files
///file 1
   TFile f1("~/cernbox/HN_Reload/HCMN/test_plots/SR_TTST_2016_HLTMu50.root");
   f1.ls();
 //  f1.cd("M_llBoostJ;1"); 
   TH1D *h1; gDirectory->GetObject("M_llBoostJ", h1);
  /* h1->Sumw2(); //normalization
   double w1 = h1->Integral();//GetEntries();
   h1->Scale(1./w1);*/
   //file 2     
   TFile f2("~/cernbox/HN_Reload/HCMN/test_plots/SR_DY_2016_HLTMu50.root");
   f2.ls();
  // f2.cd("M_llBoostJ;1"); 
   TH1D *h2; gDirectory->GetObject("M_llBoostJ", h2);
/*   h2->Sumw2(); //normalization
   double w2 = h2->Integral();//GetEntries();
   h2->Scale(1./w2);*/
//file 2     
   TFile f2b("~/cernbox/HN_Reload/HCMN/test_plots/SR_Other_2016_HLTMu50.root");
   f2b.ls();
//   f2b.cd("M_llBoostJ;1"); 
   TH1D *h2b; gDirectory->GetObject("M_llBoostJ", h2b);
/*   h2b->Sumw2(); //normalization
   double w2b = h2b->Integral();//GetEntries();
   h2b->Scale(1./w2b);*/
  //file 3     
   TFile f3("~/cernbox/HN_Reload/HCMN/test_plots/SR_mumujjL13M1000_2016_HLTMu50.root");
   f3.ls();  
 //  f3.cd("M_llBoostJ;1"); 
   TH1D *h3; gDirectory->GetObject("M_llBoostJ", h3);
...
///////
 ...
 //////
 double Nb=h1->GetNbinsX();
double X=h1->GetBinCenter(9);
double Bincont=h1->GetBinContent(9);
double Y=h1->GetBinContent(9) + h2b->GetBinContent(9) + h2->GetBinContent(9);
double Xerr=h1->GetBinWidth(9)/2;
double Yerr=sqrt(h1->GetBinContent(9) + h2->GetBinContent(9) + h2b->GetBinContent(9) )/2;
 cout << Nb<< endl;
 cout << Bincont<< endl;
 cout << X << endl;
 cout << Y << endl;
...
//  	all_bkg_statErr_x[m]=h1->GetBinCenter(m) ;
  ...
  //	all_bkg_statErr_xerr[m]=h1->GetBinWidth(m) / 2 ;
    ...
    ///statistic
  	all_bkg_statErr_yerr[m-1]=sqrt( (h1->GetBinContent(m))*(h1->GetBinContent(m)) + (h2->GetBinContent(m))*(h2->GetBinContent(m)) + (h2b->GetBinContent(m))*(h2b->GetBinContent(m)) );
  	///systematics
 	all_bkg_sistErr_yerr[m-1]=sqrt( 	 (h1->GetBinContent(m))*(h1->GetBinContent(m))*1/100  +   (h2->GetBinContent(m))*(h2->GetBinContent(m))*1/100  +  (h2b->GetBinContent(m))* (h2b->GetBinContent(m))*1/100   //lumi
  													+  (h1->GetBinContent(m))*(h1->GetBinContent(m))*2/100  +   (h2->GetBinContent(m))*(h2->GetBinContent(m))*2/100  +   (h2b->GetBinContent(m))*(h2b->GetBinContent(m))*2/100   //pileup	
  													+ (h1->GetBinContent(m))*(h1->GetBinContent(m))*0.5/100  +  (h2->GetBinContent(m))* (h2->GetBinContent(m))*0.5/100  +   (h2b->GetBinContent(m))*(h2b->GetBinContent(m))*0.5/100   //ele ID
													+ (h1->GetBinContent(m))*(h1->GetBinContent(m))*0.5/100  +  (h2->GetBinContent(m))* (h2->GetBinContent(m))*0.5/100  +   (h2b->GetBinContent(m))*(h2b->GetBinContent(m))*0.5/100   //ele scale 
													+ (h1->GetBinContent(m))*(h1->GetBinContent(m))*0.5/100  +   (h2->GetBinContent(m))*(h2->GetBinContent(m))*0.5/100  +  (h2b->GetBinContent(m))* (h2b->GetBinContent(m))*0.5/100   // muon ID 
													+ (h1->GetBinContent(m))*(h1->GetBinContent(m))*0.5/100  +  (h2->GetBinContent(m))* (h2->GetBinContent(m))*0.5/100  +  (h2b->GetBinContent(m))* (h2b->GetBinContent(m))*0.5/100   //muon scale	
													+ (h1->GetBinContent(m))*(h1->GetBinContent(m))*1/100  + (h2->GetBinContent(m))*  (h2->GetBinContent(m))*1/100  +   (h2b->GetBinContent(m))*(h2b->GetBinContent(m))*1/100   //JES 			
													+ (h1->GetBinContent(m))*(h1->GetBinContent(m))*1/100  +  (h2->GetBinContent(m))* (h2->GetBinContent(m))*1/100  +  (h2b->GetBinContent(m))* (h2b->GetBinContent(m))*1/100   //JER 	
													+ (h1->GetBinContent(m))*(h1->GetBinContent(m))*0.3/100  +   (h2->GetBinContent(m))*(h2->GetBinContent(m))*0.3/100  +  (h2b->GetBinContent(m))* (h2b->GetBinContent(m))*0.3/100   //Bkg	   if this is v2->0.3, otherwise 5%
													+   (h2->GetBinContent(m))*(h2->GetBinContent(m))*4/100  );   //DY th 	
  	/////total error
  	all_bkg_totErr_yerr[m-1]=sqrt( (all_bkg_statErr_yerr[m-1] )*(all_bkg_statErr_yerr[m-1]) +  (all_bkg_sistErr_yerr[m-1])*(all_bkg_sistErr_yerr[m-1]) );  		
   }
...
//	latexLabel2.SetTextFont(32);
	latexLabel.SetNDC();
	latexLabel.DrawLatex(0.15, 0.93, "CMS Run 2 (Preliminary)");
	//Phase-2 simulation #it{#bf{Preliminary}});
	TLatex latexLabel2;
	latexLabel2.SetTextSize(0.039);
//	latexLabel2.SetTextFont(32);
	latexLabel2.SetNDC();
	latexLabel2.DrawLatex(0.76, 0.93, "35.9 fb^{-1} (13 TeV)");
// poi disegni la legenda  
TLegend *leg = new TLegend(0.5, 0.68, 0.91, 0.88);   //x2=0.59 per 0 PU, x2=0.65 per 200PU linear, x2=0.59 per 200PU log
...
//	leg->AddEntry(h6b, "All bkds", "l");
	leg->AddEntry(h3, "#Lambda = 13 TeV, M(N_{e}) = 1 TeV",  "l");	
	leg->AddEntry(all_bkg_statErr,"#scale[0.9]{Bkg stat. and syst. uncert.}","F");
