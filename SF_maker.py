...
with open("RunBCDEF_SF_ID.json") as f1:
    test = json.load(f1)


print ("eta_low;eta_high;pt_low;pt_high;value;stat;syst")
for key, value in test["NUM_LooseID_DEN_genTracks"]["abseta_pt"].items():

    eta = key.split(":")[1]
    eta_low = eta.split("[")[1].split(',')[0]
    eta_high = eta.split("]")[0].split(',')[1]

    print("if(fabs(eta)>%s && fabs(eta)<=%s){" % (eta_low, eta_high))


    for pt_int, dic in value.items():
      ...
      # syst = dic["syst"]
        ...
        print("    if(pt > %s && pt <= %s) musf=%s; // error %s " % (pt_low, pt_high, value, error))
