 ...
 if (Muon_pt_tunePbt_Roc->size() > 0 && patElectron_pt->size() > 0 && numOfHighptMu==1 && numOfHighptEle == 1 && numOfBoostedJets>=1){
  ...
  if (HLT_Mu == 1 && LeadLep.Pt() > 150 && SubLeadLep.Pt() > 100 && fabs(Muon_eta->at(0))<2.4 && fabs(patElectron_eta->at(0))<2.4
   ...
   eejj_L13_M1000_TTtWcr_ll_energyScaleUp->Fill((Ele_ScaleUp+LeadLep).M(),wg);
    ...
    if ((Muon_PtResoUp+SubLeadLep).M() >300) eejj_L13_M1000_TTtWcr_ll_2017_PtResoUp->Fill((Muon_PtResoUp+SubLeadLep).M(), wg);
    eejj_L13_M1000_TTtWcr_ll_2017_PtResoDown->Fill((LeadLep+SubLeadLep).M(), wg);
   ...
   if (veto_ele == false){
    ...
    if ((Muon_PtResoUp+LeadLep).M() >300) eejj_L13_M1000_TTtWcr_ll_2017_PtResoUp->Fill((Muon_PtResoUp+LeadLep).M(), wg);
    eejj_L13_M1000_TTtWcr_ll_2017_PtResoDown->Fill((SubLeadLep+LeadLep).M(), wg);
    M_leplepJ_TTtWcr->Fill((LeadLep+SubLeadLep+BoostJet).M(),wg);
