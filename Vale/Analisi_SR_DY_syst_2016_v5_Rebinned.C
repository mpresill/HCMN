...
//      cout << "k_ewk = "<<k_ewk << ",  bin number = "<< jj << ", bin center = "<< k_ewk_histo->GetXaxis()->GetBinCenter(jj)<<", bin content = "<< k_ewk_histo->GetBinContent(jj) << "Gen_stat = " << Gen_stat <<endl;
 ...
 if (Muon_pt_tunePbt->size() > 1 && numOfHighptMu==2 && numOfVetoEle == 0 && numOfBoostedJets>=1){
  ...
  Muon1M.SetPtEtaPhiM(Muon_pt_tunePbt->at(0), Muon_eta->at(0), Muon_phi->at(0),0.1056583745);
  Muon2M.SetPtEtaPhiM(Muon_pt_tunePbt->at(1), Muon_eta->at(1), Muon_phi->at(1),0.1056583745);
  Muon1_PtResoUpM.SetPtEtaPhiM((Muon_pt_tunePbt->at(0)+0.1*Muon_pt_tunePbt->at(0)), Muon_eta->at(0), Muon_phi->at(0),0.1056583745 );
  Muon2_PtResoUpM.SetPtEtaPhiM((Muon_pt_tunePbt->at(1)+0.1*Muon_pt_tunePbt->at(1)), Muon_eta->at(1), Muon_phi->at(1),0.1056583745 );
  Muon1_PtResoDownM.SetPtEtaPhiM((Muon_pt_tunePbt->at(0)-0.1*Muon_pt_tunePbt->at(0)), Muon_eta->at(0), Muon_phi->at(0),0.1056583745 );
  Muon2_PtResoDownM.SetPtEtaPhiM((Muon_pt_tunePbt->at(1)-0.1*Muon_pt_tunePbt->at(1)), Muon_eta->at(1), Muon_phi->at(1),0.1056583745 );
  end testing*/
...
const double Alpha_ele[9] =  {1,1,1.09,1.14,1.28,1.35,1.18,1.92 };
const double dAlpha_ele[9] = {0,0,0.06,0.03,0.04,0.06,0.12,0.52 };
...
TFile *f = new TFile("/eos/user/m/mpresill/CMS/HN_Reload/combine_histograms/2016_ALL_HOPE/SR_syst_DY_HTincl_Kewkqcd_2016.root", "RECREATE");
