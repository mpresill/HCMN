...
TH1F * gHisto ;
   ...
   TFile *f1 = new TFile("/afs/cern.ch/user/v/vmariani/CMSSW_10_2_16_UL/src/BSMFramework/BSM3G_TNT_Maker/python/2016/SR_syst_TTtW_2016_eta_regions.root");
   TFile *f2 = new TFile("/afs/cern.ch/user/v/vmariani/CMSSW_10_2_16_UL/src/BSMFramework/BSM3G_TNT_Maker/python/2016/SR_syst_DY_HTincl_Kewkqcd_2016_eta_regions.root");
   TFile *f3 = new TFile("/afs/cern.ch/user/v/vmariani/CMSSW_10_2_16_UL/src/BSMFramework/BSM3G_TNT_Maker/python/2016/SR_syst_Other_2016_eta_regions.root");
   TFile *f4 = new TFile("/afs/cern.ch/user/v/vmariani/CMSSW_10_2_16_UL/src/BSMFramework/BSM3G_TNT_Maker/python/2016/SR_syst_eejj_L13_M1000_2016_eta_regions.root");
   TFile *f5 = new TFile("/afs/cern.ch/user/v/vmariani/CMSSW_10_2_16_UL/src/BSMFramework/BSM3G_TNT_Maker/python/2016/CR_TTtW_data_ele_2016_eta_regions.root");

   TString name = "Mll_TTcr_2016_BB"; //nome del file salvato

/////input files
  //2016
   TH1F *h1_c= (TH1F*) f1->Get("TTtW_TTtWcr_ll_BB");
   TH1F *h2_c= (TH1F*) f2->Get("DY_TTtWcr_ll_BB");
   TH1F *h2b_c= (TH1F*) f3->Get("Other_TTtWcr_ll_BB");
   TH1F *h4_c= (TH1F*) f4->Get("eejj_L13_M1000_TTtWcr_ll_BB");
   TH1F *h5_c= (TH1F*) f5->Get("data_obs_BB");
...
for(int i=1;i<w;i++){
   ...
   all_bkg_Err_x[i] = h1->GetBinCenter(i);
   all_bkg_Err_y[i] = h1->GetBinContent(i)+h2b->GetBinContent(i)+h2->GetBinContent(i);
   all_bkg_Err_xerr[i] = h1->GetBinWidth(i)/2;
...
// definisci gli assi dell'histo che disegni per primo, con titolo offset etc
    hs->GetXaxis()->SetLimits(300,2000);
...
for(int j=1; j<=w; j++){
 ...
 cout << "x " << dataSUmc_x[j] << " data " << rd << " mc " << mc << " ratio " << dataSUmc_y[j] << endl;
}

TGraphErrors *dataSUmc = new TGraphErrors(w+1, dataSUmc_x, dataSUmc_y, dataSUmc_xerr, dataSUmc_yerr);

 dataSUmc->GetXaxis()->SetLimits(300,2000);
dataSUmc->Draw("APZ");
dataSUmc->SetTitle(0);
dataSUmc->SetMarkerStyle(8);
dataSUmc->GetYaxis()->SetNdivisions(5,5,1);

dataSUmc->GetXaxis()->SetTitle("M(l,l) (GeV)");
...
canv->Print("/eos/user/v/vmariani/www/HN/eta_regions/Prefiring/Mll_TTtWcr_2016_BB.png");
canv->Print("/eos/user/v/vmariani/www/HN/eta_regions/Prefiring/Mll_TTtWcr_2016_BB.pdf");
  return canv;
}
