 ...
 if (Muon_pt_tunePbt->size() > 0 && patElectron_pt->size() > 0 && numOfHighptMu==1 && numOfHighptEle == 1 && numOfBoostedJets>=1){
  ...
  if (HLT_Mu == 1 && LeadLep.Pt() > 150 && SubLeadLep.Pt() > 100 && fabs(Muon_eta->at(0))<2.4 && fabs(patElectron_eta->at(0))<2.4
   ...
   if (veto_ele == false){
    ...
    if(JesSFup->at(1)==1)TTtW_TTtWcr_llJ_2016_JesSFUp->Fill((LeadLep+SubLeadLep+BoostJet).M(), wg);
    if(JesSFdown->at(2)==1)TTtW_TTtWcr_llJ_2016_JesSFDown->Fill((LeadLep+SubLeadLep+BoostJet).M(), wg);
    if(JerSFup->at(3)==1)TTtW_TTtWcr_llJ_2016_JerSFUp->Fill((LeadLep+SubLeadLep+BoostJet).M(), wg);
    if(JerSFdown->at(4)==1)TTtW_TTtWcr_llJ_2016_JerSFDown->Fill((LeadLep+SubLeadLep+BoostJet).M(), wg);
