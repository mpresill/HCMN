...
//void filename_()
void Analisi_CR_DY_data_ele_2017_v5_Rebinned(){
 ...
 if (Muon_pt_tunePbt->size() > 1 && numOfHighptMu==2 && numOfVetoEle == 0 && numOfBoostedJets>=1){
  ...
  if(mmumu >50 && mmumu < 130){
   ...
   if (mmumu >=150 && mmumu <= 300){
    ...
    if (Muon_charge->at(0)*Muon_charge->at(1) == 1) M_mumuJ_SS->Fill((Muon1+Muon2+BoostJet).M());
    if (Muon_charge->at(0)*Muon_charge->at(1) == -1) M_mumuJ_OS->Fill((Muon1+Muon2+BoostJet).M()); 
