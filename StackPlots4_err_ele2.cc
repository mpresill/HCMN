...
const char *samples[]   = {"TTtW","DY","Other","L9000_M1500","L9000_M2500","data_ele"};//,"L9000_M500","L9000_M1500","L9000_M2500","data"};
const string channel    = "eejj";
const string selection  = "_SRe"; //_SingleEle, _SingleMu
...
//Errori sistematici
const double PU_Other[numBin] = {0,0,42.396,10.7286,6.42659,2.00847,11.4141,12.201,0};//,0};
const double PU_DY[numBin] = {0,0,10.7075,5.48586,6.47114,1.6904,4.42663,1.70586,13.2519};//,0};
const double PU_TTtW[numBin] = {0,0,10.0228,2.09384,3.42342,3.76729,7.71348,0,0};//,0};
const double muSF_TTtW[numBin] = {0,0,5.71654,3.26795,3.78636,3.50763,5.6863,0,0};//,0};
const double eleSF_Other[numBin] = {0,0,2.59456,2.36978,2.36224,2.53486,2.862,2.67401,0};//,0};
const double eleSF_DY[numBin] = {0,0,0.660081,0.569146,0.490892,0.485693,0.497843,0.42725,1.0234};//,0};
const double eleSF_TTtW[numBin] = {0,0,0.962313,1.53519,1.60945,1.53025,1.45189,0,0};//,0};
const double JES_Other[numBin] = {0,0,0,0,0.194103,1.89241,3.66857,0,0};//,0};
const double JES_DY[numBin] = {0,0,0.925927,11.179,5.63075,4.80233,0.809669,1.91782,0.925928};//,0};
const double JES_TTtW[numBin] = {0,0,55.5157,6.56691,1.88404,7.19264,1.95149,0,0};//,0};
const double JER_Other[numBin] = {0,0,0,0,0,1.34804,3.66857,0,0};//,0};
const double JER_DY[numBin] = {0,0,0,5.11204,0.206624,0.155205,0.162769,1.11595,0};//,0};
const double JER_TTtW[numBin] = {0,0,1.85186,1.5873,0,1.72801,1.5625,0,0};//,0};
const double eleScale_Other[numBin] = {0,0,0,1.84233,1.03448,1.46368,3.88748,0,0};//,0};
const double eleScale_DY[numBin] = {0,0,40.0636,10.6722,8.77173,6.49896,3.76843,3.01317,0.925928};//,0};
const double eleScale_TTtW[numBin] = {0,0,54.6207,3.5615,0.945447,1.69707,4.75676,0,0};//,0};
const double muScale_TTtW[numBin] = {0,0,49.0985,3.46952,0.989899,3.40003,0.00540048,0,0};//,0};
//const double QCDest[numBin] = {0,0,100,100,100,100,100,100,100,100};
const double DYest[numBin] = {0,0,100,45.451,42.5076,49.1479,72.9674,100,100};//,0};
const double DYth[numBin] = {0,0,6.99589,7.00777,8.65076,7.8155,25.2318,11.2698,20.2095};//,0};
const double TTtWest[numBin] = {0,0,74.8264,16.6325,16.4605,18.9473,31.3866,0,0};//,0}; 
...
void StackPlots4_err_ele2(){
 ...
 for(uint v=ini_var; v<fin_var; v++){
  ...
  for(uint i=0; i<rootplas_size; i++){
   ...
   if(rootplas[i]=="L9000_M1500" || rootplas[i]=="L9000_M2500" || rootplas[i]=="L8000_M500" || rootplas[i]=="L8000_M1500" || rootplas[i]=="L8000_M2500"){                                                 datatype = 1; //for signal
   ...
   if(datatype==2){       
    ...
    // else{ 
      ...
      if(rootplas[i]=="Other"){
       for(int ii=1; ii<=bin[v]; ii++){
        h_syst->SetBinContent(ii,pow(h_var->GetBinContent(ii)*PU_Other[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*eleSF_Other[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*JES_Other[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*JER_Other[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*eleScale_Other[ii-1]/100,2));
       }
      }
      if(rootplas[i]=="TTtW"){
       for(int ii=1; ii<=bin[v]; ii++){
        h_syst->SetBinContent(ii,pow(h_var->GetBinContent(ii)*PU_TTtW[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*eleSF_TTtW[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*muSF_TTtW[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*JES_TTtW[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*JER_TTtW[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*TTtWest[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*eleScale_TTtW[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*muScale_TTtW[ii-1]/100,2));
       }
      }
      if(rootplas[i]=="DY"){
       for(int ii=1; ii<=bin[v]; ii++){
        h_syst->SetBinContent(ii,pow(h_var->GetBinContent(ii)*PU_DY[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*eleSF_DY[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*JES_DY[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*JER_DY[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*DYest[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*eleScale_DY[ii-1]/100,2)+pow(h_var->GetBinContent(ii)*DYth[ii-1]/100,2));
       }
      }
...
void draw_plots(TCanvas* c1, TH1F* h_sum_var, THStack* hstack, TH1F* h_data_var, TH1F* h_data_var2, TH1F* h_sig, TH1F* h_sig2, TH1F* h_sig3, TH1F* h_sig4, TH1F* h_sig5, TH1F* h_sum_err, TH1F* h_sum_syst, TH1F* h_TTtW, TH1F* h_DY, TH1F* h_Other, TLegend* leg, double err_AllBkg[][col_size], double ent_AllBkg[][col_size], uint rootplas_size, int v, string var, string vartitle, double highestbinval){
 ...
 string sig_leg = "#scale[0.75]{#Lambda = 9 TeV, #font[12]{m}_{N_{e}}= 1.5 TeV}";
 ...
 string sig2_leg = "#scale[0.75]{#Lambda = 9 TeV, #font[12]{m}_{N_{e}}= 2.5 TeV}";
