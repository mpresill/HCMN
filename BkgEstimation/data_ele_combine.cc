...
void data_ele_combine(){
 ...
 //TCanvas* c1 = new TCanvas("SRshape","SRshape",400,400,1100,800);
  //Declare histos
  TH1F *data_ele; TH1F *DY; TH1F *DYstim; TH1F *TT; TH1F *tW; TH1F *TTtW; TH1F *data_read; TH1F *data_obs; TH1F *data_sub; TH1F *sig; TH1F *data_obs2;
  if(!asymbin){
   data_obs    = new TH1F("","",bin,inRange,endRange);
   data_obs2   = new TH1F("data_obs","data_obs",bin,inRange,endRange); 
  }else{
   data_obs    = new TH1F("","",bin,asymbins);
   data_obs2   = new TH1F("data_obs","data_obs",bin,asymbins);
  }
  for(uint i=0; i<rootplas.size(); i++){
   if(rootplas[i]=="data_ele") data_ele = get_treehist(rootplas[i],i);
   }
  /*
  ...
  */
  for(int i=1; i<=bin; i++){
    data_obs2->SetBinContent(i,data_ele->GetBinContent(i));
    if(data_obs2->GetBinContent(i)!=0){
     data_obs2->SetBinError(i,data_ele->GetBinError(i));
    }
    else{data_obs2->SetBinError(i,0);}
    cout<<"Bin"<<i<<"="<<data_obs2->GetBinContent(i)<<endl;
    cout<<"Err"<<i<<"="<<data_obs2->GetBinError(i)<<endl;
  } 
  //Create new file
  string norm;
  if(normalize) norm = "_norm";
  string newfilename = "combine_histograms/data_obs_ele_2016"+norm+".root";
  TFile *newfile = new TFile(newfilename.c_str(),"recreate");
  data_obs2->Write();  delete data_obs2;
