...
//const char *ParamPoints[] = {"L5000_M500", "L5000_M1500", "L5000_M2500", "L5000_M3500", "L5000_M4500", 
                           ...
                           // };
const char *samples[]     = {"DY","Other","data_mu"};
const string selection    = "_TRmu";  
//Plots option
...
const double TTtWSF[bin]     = {1,1,1,1,1,1.93,1.15,0.99,1};//For mu-channel with summed TT and tW
const double SFerr[bin]      = {0,0,0,0,0,1.27,0.63,0.72,0};//For mu-channel with summed TT and tW
...
void TTtW_Mu_Estimation(){
 ...
 //for(uint pp=0; pp<parampoints.size(); pp++){
  ...
  for(uint i=0; i<rootplas.size(); i++){
   ...
   if(rootplas[i]=="data_mu")   data_read = get_treehist(rootplas[i],i); 
  ...
  for(int i=1; i<=bin; i++){
   if(data_read->GetBinContent(i)>=0){
    ...
    /*
     ...
     if(i==9) data_sub->SetBinError(i,0.0041);
     //if(i==10) data_sub->SetBinError(i,0.000030);
     */
    }
  ...
  string newfilename = "mumujj_TTtW"+norm+".root";
