...
const double DYSF            = 0.897;
const double DYSFerr         = 0.009;
...
void DY_Ele_Estimation(){
 ...
 //TCanvas* c1 = new TCanvas("SRshape","SRshape",400,400,1100,800);
  //Declare histos
  TH1F *DY; TH1F *DYstim; TH1F *TT; TH1F *tW; TH1F *TTtW; TH1F *Other; TH1F *data_read; TH1F *data_obs; TH1F *data_sub; TH1F *sig;
  if(!asymbin){
   DY    = new TH1F("","",bin,inRange,endRange);
   DYstim = new TH1F("DY","DY",bin,inRange,endRange);
  }else{
   DY    = new TH1F("","",bin,asymbins);
   DYstim = new TH1F("DY","DY",bin,asymbins);
  }
  //Fill histos
  ...
  for(int j=1; j<=bin; j++){
   ...
   double syst;
   if(bincont>0){
    syst=100*(sqrt(pow((binerr*DYSF),2)+pow((bincont*DYSFerr),2)))/bincont;
   }
   else syst=0;
   cout<<syst<<",";
  }
  cout<<"}"<<endl;
  for(int i=1; i<=bin; i++){
    DYstim->SetBinContent(i,DY->GetBinContent(i)*DYSF);
    if(DYstim->GetBinContent(i)!=0){
     DYstim->SetBinError(i,sqrt((pow((DY->GetBinError(i)/DY->GetBinContent(i)),2)+pow((DYSFerr/DYSF),2)))*(DYstim->GetBinContent(i)));
    }
    else{
     DYstim->SetBinError(i,0);
     //if(i==10) DYstim->SetBinError(i,0.0000041);
    }
    cout<<"Bin"<<i<<"="<<DYstim->GetBinContent(i)<<endl;
    cout<<"Err"<<i<<"="<<DYstim->GetBinError(i)<<endl;
  } 
  //Create new file
  string norm;
...
TH1F* get_treehist(string rootpla, int idx){
 ...
 TH1F *hist;
 string namehist;
 if(rootpla.substr(0,1)=="L") namehist = "DY";
