...
void DY_Ele_Estimation_MC(){
 ...
 //TCanvas* c1 = new TCanvas("SRshape","SRshape",400,400,1100,800);
  //Declare histos
  TH1F *DY; TH1F *DYstim; TH1F *TT; TH1F *tW; TH1F *TTtW; TH1F *Other; TH1F *data_read; TH1F *data_obs; TH1F *data_sub; TH1F *sig; TH1F *DY2;
  if(!asymbin){
   DY    = new TH1F("","",bin,inRange,endRange);
   DY2   = new TH1F("DY","DY",bin,inRange,endRange); 
  }else{
   DY    = new TH1F("","",bin,asymbins);
   DY2   = new TH1F("DY","DY",bin,asymbins);
  }
  ...
  */
  for(int i=1; i<=bin; i++){
    DY2->SetBinContent(i,DY->GetBinContent(i));
    if(DY2->GetBinContent(i)!=0){
     DY2->SetBinError(i,DY->GetBinError(i));
    }
    else{DY2->SetBinError(i,0);}
    cout<<"Bin"<<i<<"="<<DY2->GetBinContent(i)<<endl;
    cout<<"Err"<<i<<"="<<DY2->GetBinError(i)<<endl;
  } 
  //Create new file
  string norm;
  if(normalize) norm = "_norm";
  string newfilename = "combine_histograms/eejj_DY_2016"+norm+".root";
  TFile *newfile = new TFile(newfilename.c_str(),"recreate");
  DY2->Write();  delete DY2;
